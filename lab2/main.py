...
def remove_stopwords(data):
    ...
    data = data.replace(',', '')
    data = data.replace('.', '')
    data = ' '.join(i for i in data.split() if i not in stopwords and len(i)!=2)
    return data
...
def process_data(data_list):
    ...
    return result_data_list,number_of_words,len(word_frequency.keys())
...
def main():
    ...
    word_frequency_by_class, number_of_words, word_count = process_data(train_data_list)
